...
interface PopoverConfig {
  horizontalAlign: CePopoverHorizontalAlign;
  verticalAlign: CePopoverVerticalAlign;
  hasBackdrop: boolean;
  backdropClass: string;
  scrollStrategy: CePopoverScrollStrategy;
...
export class PopoverAnchoringService implements OnDestroy {
  ...
  /** Opens the popover. */
  openPopover(options: CePopoverOpenOptions = {}): void {
  ...
  private _applyOpenOptions(options: CePopoverOpenOptions): void {
    // Only override restoreFocus as `false` if the option is explicitly `false`
    this._popover._restoreFocusOverride = options.restoreFocus !== false;

    // Only override autoFocus as `false` if the option is explicitly `false`
    this._popover._autoFocusOverride = options.autoFocus !== false;
  }
  ...
  private _getScrollStrategyInstance(
    strategy: CePopoverScrollStrategy,
  ): ScrollStrategy {
    switch (strategy) {
      case 'block':
  ...
  private _getPositionStrategy(
    horizontalTarget: CePopoverHorizontalAlign,
    verticalTarget: CePopoverVerticalAlign,
    forceAlignment: boolean,
    lockAlignment: boolean,
    anchor: HTMLElement,
  ): FlexibleConnectedPositionStrategy {
  ...
  private _getFallbacks(
    hTarget: CePopoverHorizontalAlign,
    vTarget: CePopoverVerticalAlign,
  ): ConnectionPositionPair[] {
    ...
    // cover the anchor
    const possibleHorizontalAlignments: CePopoverHorizontalAlign[] = horizontalOverlapAllowed
      ? ['before', 'start', 'center', 'end', 'after']
      : ['before', 'after'];
    const possibleVerticalAlignments: CePopoverVerticalAlign[] = verticalOverlapAllowed
...
function getPosition(
  h: CePopoverHorizontalAlign,
  v: CePopoverVerticalAlign,
): ConnectionPositionPair {
...
function getHorizontalPopoverAlignment(
...
/** Helper function to convert an overlay connection position to equivalent popover alignment. */
function getVerticalPopoverAlignment(
  v: VerticalConnectionPos,
): CePopoverVerticalAlign {
  if (v === 'top') {
    return 'below';
  }
