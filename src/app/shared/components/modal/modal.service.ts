...
import { IModalOptions, ModalConfig } from './modal-config';
...
export class ModalService {
  ...
  public resetModalData(id: string): any | boolean {
    if (
      !!this._modalStack.getModalStack().find((o: ModalInstance) => o.id === id)
    ) {
  ...
  private _addEvents(): boolean {
    ...
    window.addEventListener(ModalConfig.prefixEvent + 'create', ((
      e: CustomEvent,
    ) => {
      this._initModal(e.detail.instance);
    }) as EventListener);

    window.addEventListener(ModalConfig.prefixEvent + 'delete', ((
      e: CustomEvent,
    ) => {
      this._deleteModal(e.detail.instance);
    }) as EventListener);

    window.addEventListener(ModalConfig.prefixEvent + 'open', ((
      e: CustomEvent,
    ) => {
      this._openModal(e.detail.instance.modal, e.detail.top);
    }) as EventListener);

    window.addEventListener(ModalConfig.prefixEvent + 'toggle', ((
      e: CustomEvent,
    ) => {
      this._toggleModal(e.detail.instance.modal, e.detail.top);
    }) as EventListener);

    window.addEventListener(ModalConfig.prefixEvent + 'close', ((
      e: CustomEvent,
    ) => {
      this._closeModal(e.detail.instance.modal);
    }) as EventListener);

    window.addEventListener(ModalConfig.prefixEvent + 'dismiss', ((
      e: CustomEvent,
    ) => {
  ...
  private _openModal(modal: ModalComponent, top?: boolean): boolean {
    ...
    setTimeout(() => {
      ...
      modal.ceDialog.first.nativeElement.setAttribute('role', 'dialog');
      modal.ceDialog.first.nativeElement.setAttribute('tabIndex', '-1');
      modal.ceDialog.first.nativeElement.setAttribute('aria-modal', 'true');
      modal.ceDialog.first.nativeElement.focus();
  ...
  private _closeModal(modal: ModalComponent): boolean {
    ...
    setTimeout(() => {
      ...
      modal.ceDialog.first.nativeElement.removeAttribute('tabIndex');
      modal.markForCheck();
  ...
  private _trapFocusModal = (event: KeyboardEvent) => {
    if (event.key === 'Tab') {
      try {
        ...
        if (
          !modal.ceDialog.first.nativeElement.contains(document.activeElement)
        ) {
